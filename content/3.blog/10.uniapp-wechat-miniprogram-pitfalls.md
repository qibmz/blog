---
title: 'UniApp 踩坑记录'
description: '记录 UniApp 开发中遇到的典型问题：uni.showModal 按钮文字字数限制、iOS 端元素溢出视口、picker 组件在 iOS 端的时区问题、以及 picker 未选中项背景颜色修改。'
date: '2026-04-07'
image:
...
- **样式不能使用 scoped**：`picker-mask-transparent` 这个 class 需要定义在全局样式中，或者在 `<style>` 标签中不添加 `scoped` 属性
