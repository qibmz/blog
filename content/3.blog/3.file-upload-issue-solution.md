...
在使用 uni-app 开发过程中，我们经常会遇到文件上传的需求。然而，[uni.uploadFile](https://uniapp.dcloud.net.cn/api/request/network-file.html#uploadfile) 方法存在一个限制：它无法在一个请求中上传多个文件。本文将介绍如何在 H5 环境下使用 [fetch](https://developer.mozilla.org/zh-CN/docs/Web/API/Fetch_API) + [FormData](https://developer.mozilla.org/zh-CN/docs/Web/API/FormData) 来解决这个问题。
...
```ts
// 在 H5 环境下上传多文件
function uploadMultipleFiles(files, uploadUrl) {
  // 检查是否在 H5 环境中
  if (__PLATFORM__ !== 'h5') {
...
```javascript
// 带进度的上传实现
function uploadMultipleFilesWithProgress(files, uploadUrl, onProgress) {
