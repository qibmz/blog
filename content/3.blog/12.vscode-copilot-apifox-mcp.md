---
title: VS Code Copilot 配置 Apifox MCP：让 AI 直接读 API 文档
description: Apifox 官方未写清 VS Code Copilot 的 MCP 接入步骤。本文给出 mcp.json 配置、鉴权与验证方法，让 Copilot 直接读取 Apifox API 文档。
date: 2026-05-19
badge:
  label: 工具
image:
  src: https://pub-6d1497ab7602406d9ac97574064124fe.r2.dev/blogs/vscode-copilot-apifox-mcp.webp
authors:
  - name: qibmz
...
4. 在弹出的选择框中选择第一个选项：**命令 (stdio)**
5. 输入命令：`npx`
6. 输入服务器 ID：`apifox`（如果存在多个 Apifox 文档，需要注意命名避免冲突，如 `apifox-user`、`apifox-order` 等）
7. 选择配置范围：**全局** 或 **本地**（根据自己需要选择）
8. VS Code 会自动打开 `mcp.json` 配置文件
