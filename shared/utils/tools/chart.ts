...
donut 时每个扇区应使用不同颜色：可在 data 各项写 color（如 #3b82f6），或在 series 里为每个扇区准备不同 color。
当用户要求画图、看趋势、对比数据、看占比时调用此工具，不要只用 markdown 表格代替。`,
  inputSchema: z.object({
    ...
    data: z.array(z.record(z.string(), z.union([z.string(), z.number()]))).min(1)
      .describe('数据点数组。line/area/bar：每项含 xKey 与所有 series.key；donut：每项是一个扇区，含 xKey（名称）、series[0].key（数值），建议再含 color（十六进制）以区分扇区'),
    xKey: z.string().describe('横轴/扇区名称字段，例如 month、date、name'),
    series: z.array(chartSeriesSchema).min(1)
      .describe('系列配置。line/area/bar 可多系列；donut 若未在 data 写 color，请为每个扇区提供不同 color 的 series 项'),
